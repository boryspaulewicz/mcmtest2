...
## Wersja do badania prof. Łosiaka - nie chcemy powtarzać słów, które już były. Połowa nowych
...
## Dobieramy słowa na podstawie przebiegu poprzedniej sesji
...
if(dim(used) == 0){
    db.disconnect()
    gui.error.msg('Nie ma osoby o tym identyfikatorze w bazie danych', quit.after = T)
}
neg = tolower(as.character(d$słowo[(d$emocja == 'neg') & (d$rodzaj == 'p')]))
neg = sample(neg[!(neg %in% as.character(used$word))], NOF.ITEMS)
neu = tolower(as.character(d$słowo[(d$emocja == 'neu') & (d$rodzaj == 'p')]))
neu = sample(neu[!(neu %in% as.character(used$word))], NOF.ITEMS)
pos = tolower(as.character(d$słowo[(d$emocja == 'poz') & (d$rodzaj == 'p')]))
pos = sample(pos[!(pos %in% as.character(used$word))], NOF.ITEMS)
db.disconnect()
